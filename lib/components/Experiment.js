...
export class Experiment {
   ...
   * @param {boolean} cfg.supervised Use supervised version of experiment (no consent)
   * @param {boolean} cfg.demo Use demo version of experiment (no consent, no Firebase)
   * @param {boolean} cfg.noPoints Hide the points and "worker" panel
   ...
   * @param {number} cfg.cameraLayout (VR) Set monocular or binocular vision (0: monocular left, 1: monocular right, 2: binocular)
   */
  constructor(cfg) {
    this.cfg = cfg;
    ...
    if (!this.cfg.noPoints) {
      this.points = new Points({
        score: this.cfg.score,
        bonus: this.cfg.bonus,
      });
  ...
  async start(...loopFuncs) {
    ...
    if (numLights === 0 && !this.sceneManager.scene.environment?.isTexture) {
      console.warn(
        'You have not added any Lights to your scene! Adding one AmbientLight.'
      );
   ...
   */
  waitForConsent() {
    if (this.consented || this.cfg.demo || this.cfg.supervised) {
  ...
  async #initVR() {
    ...
    this.vrButton.style.removeProperty('width');
    setTimeout(() => (this.vrButton.style.width = '150px'), 10);
